...
# 두 모델 경로 설정 (second_checkpoints 사용)
cnn_model_path = str(project_root / "second_checkpoints" / "cnn" / "best_model.pth")
unet_model_path = str(project_root / "second_checkpoints" / "unet" / "best_model.pth")

# 모델 파일 존재 여부 확인
cnn_exists = Path(cnn_model_path).exists()
unet_exists = Path(unet_model_path).exists()

if cnn_exists:
    cnn_size = Path(cnn_model_path).stat().st_size / (1024 * 1024)  # MB
    st.sidebar.success(f"✅ CNN 모델 확인됨 ({cnn_size:.1f} MB)")
else:
    st.sidebar.error("❌ CNN 모델 파일을 찾을 수 없습니다")

if unet_exists:
    unet_size = Path(unet_model_path).stat().st_size / (1024 * 1024)  # MB
    st.sidebar.success(f"✅ U-Net 모델 확인됨 ({unet_size:.1f} MB)")
else:
    st.sidebar.error("❌ U-Net 모델 파일을 찾을 수 없습니다")
