"""
U-Net 모델 추론 및 성능 측정 스크립트 (test 데이터 + lwdds 데이터 합치기)
...
from unet_model import UNet, ImageDataset, calculate_psnr, calculate_ssim
import argparse
import time
import cv2
import numpy as np
        ...
        for idx, (noisy, clean) in enumerate(test_loader):
            ...
            for i in range(batch_size):
                ...
                if clean is not None and isinstance(clean, torch.Tensor):
                    ...
                    axes[1].set_title(f'Denoised (U-Net) #{sample_count+1}')
                    axes[1].axis('off')
...
def main():
    parser = argparse.ArgumentParser(description='U-Net 모델 추론 및 평가 (test 데이터 + lwdds 데이터 합치기)')
    ...
    parser.add_argument('--batch_size', type=int, default=2,
                        help='배치 크기 (기본값: 2, 메모리 부족 시 1로 줄이세요)')
    ...
    if args.noisy_dir:
        ...
        print(f'✅ Test 데이터셋: {test_size:,}개 쌍')
        ...
        lwdds_size = len(lwdds_dataset)
        print(f'✅ lwdds 데이터셋: {lwdds_size:,}개 쌍')
        
        # 데이터셋 합치기
        print('\n[데이터셋 합치기]')
        combined_dataset = ConcatDataset([test_dataset, lwdds_dataset])
        total_size = len(combined_dataset)
        print(f'✅ 합쳐진 데이터셋: {total_size:,}개 쌍')
        print(f'   - Test 데이터: {test_size:,}개 ({test_size/total_size*100:.1f}%)')
        print(f'   - lwdds 데이터: {lwdds_size:,}개 ({lwdds_size/total_size*100:.1f}%)')
        ...
        total_batches = len(test_loader)
        print(f'\n테스트 데이터셋: {total_size}개 이미지, {total_batches}개 배치 (batch_size={args.batch_size})')
        
        # 예상 소요 시간 계산 (U-Net은 조금 더 느릴 수 있음)
        estimated_time_per_batch = 1.5  # 초 (U-Net은 CNN보다 약간 느림)
