"""
U-Net 모델 학습 스크립트 (원래 데이터 + lwdds 데이터 합치기)
"""
import torch
...
def main():
    parser = argparse.ArgumentParser(description='U-Net 모델 학습 (원래 데이터 + lwdds 데이터 합치기)')
    ...
    # lwdds 데이터셋 파라미터
    parser.add_argument('--lwdds_noisy_dir', type=str, default='./train_img/data',
                        help='lwdds 데이터셋의 노이즈 이미지 디렉토리 (default: ./train_img/data)')
    parser.add_argument('--lwdds_clean_dir', type=str, default='./train_img/gt',
                        help='lwdds 데이터셋의 깨끗한 이미지 디렉토리 (default: ./train_img/gt)')
    ...
    print(f'\n[lwdds 데이터셋]')
    print(f'  lwdds_noisy_dir: {args.lwdds_noisy_dir}')
    print(f'  lwdds_clean_dir: {args.lwdds_clean_dir}')
    ...
    if use_split_structure:
        ...
        if args.auto_match and args.clean_dir is not None:
            ...
            # lwdds 데이터셋 로드 및 합치기
            print("\n[lwdds 데이터셋 로딩 중...]")
            lwdds_train_dataset = ImageDataset(
                args.lwdds_noisy_dir,
                args.lwdds_clean_dir,
                matched_pairs_file=None,
                auto_match=False
            )
            print(f"✅ lwdds 데이터셋: {len(lwdds_train_dataset):,}개 쌍")
            
            # 원래 Train 데이터셋에 lwdds 데이터 합치기
            original_train_size = len(train_dataset)
            train_dataset = ConcatDataset([train_dataset, lwdds_train_dataset])
            print(f"✅ 합쳐진 Train 데이터셋: {len(train_dataset):,}개 쌍")
            print(f"   - 원래 데이터: {original_train_size:,}개 ({original_train_size/len(train_dataset)*100:.1f}%)")
            print(f"   - lwdds 데이터: {len(lwdds_train_dataset):,}개 ({len(lwdds_train_dataset)/len(train_dataset)*100:.1f}%)")
        ...
        # lwdds 데이터셋 로드 및 합치기
        print("\n[lwdds 데이터셋 로딩 중...]")
        lwdds_dataset = ImageDataset(
            args.lwdds_noisy_dir,
            args.lwdds_clean_dir,
            matched_pairs_file=None,
            auto_match=False
        )
        print(f"✅ lwdds 데이터셋: {len(lwdds_dataset):,}개 쌍")
        
        # 원래 데이터셋과 합치기
        original_size = len(full_dataset)
        full_dataset = ConcatDataset([full_dataset, lwdds_dataset])
        print(f"✅ 합쳐진 데이터셋: {len(full_dataset):,}개 쌍")
        print(f"   - 원래 데이터: {original_size:,}개 ({original_size/len(full_dataset)*100:.1f}%)")
        print(f"   - lwdds 데이터: {len(lwdds_dataset):,}개 ({len(lwdds_dataset)/len(full_dataset)*100:.1f}%)")
    ...
    print(f'   python inference_unet.py --model_path {args.checkpoint_dir}/best_model.pth --noisy_dir ./test/syn/rainy_vid --clean_dir ./test/syn/clean_vid --visualize')
